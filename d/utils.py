...
def extract_patch_save_images(image, dim_patch_h, dim_patch_w, stride, resize_factor, folder_in, folder_gt, idx_image):
    h, w = image.shape
    images_in = []
    images_gt = []
    idx_image = idx_image + 1
    for i in range(0, h - dim_patch_h, stride):
        for j in range(0, w - dim_patch_w, stride):
            gt_patch = image[i:i + dim_patch_h, j:j + dim_patch_w]
            sigma = np.random.rand()
            kernel = get_kernel(3, sigma)

            if np.random.rand() < 0.5:
                in_patch = convolve(gt_patch, kernel, mode='same')
                in_patch = cv.resize(in_patch, (0, 0), fx=1 / resize_factor, fy=1)
            else:
                in_patch = cv.resize(gt_patch, (0, 0), fx=1 / resize_factor, fy=1)
